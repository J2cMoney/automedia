...
[任务]
    ...
    持续开发模式：有代码 + 有 DEV-PLAN.md → 按工作模式执行。Fast Patch 只做局部修改和最小验证；Normal Feature 按相关 Phase/Task 实现并按风险审查；Strict Release 按 Phase 逐步开发，每个 Phase 走 per-Task review→fix 循环和四步走验证。
    规划与执行方式见 AGENTS.md [规划与执行]。

[依赖检测]
    先读 .codex/workflow-mode。没有文件、空值或非法值时按 fast 处理。主 Agent 先自行判档并告知推荐档位：明显小修直接 fast，明显常规功能用 normal，明显“严格模式”“从 0 到发布”“上线/打包/部署/支付/数据库/安全”升级 strict 并写回该文件。边界不清、会显著增加耗时或涉及发布成本时，先让用户确认推荐档位。
    Skill 启动第一步执行。
...
[开发规则清单]
    ...
    [Git 工作流]
        ...
        - Fast Patch 默认不 commit、不 push，除非用户要求
        - Normal Feature 可在用户确认后 commit，不自动 push
        - Strict Release 可 commit；push 只有用户明确允许或 .codex/allow-auto-push 存在时由 hook 处理
...
[Phase 执行流程]
    Fast Patch：只读相关文件和最小上下文 → 评估影响范围 → 局部修改 → 跑最小必要验证 → 汇报改动和未跑验证。不更新 Product-Spec/DEV-PLAN，不派 code-reviewer，不自动 commit/push。
    Normal Feature：读 DEV-PLAN 相关章节、Spec 相关章节和现有代码 → 拆出本次相关 Task → 实现并自检 → 跑相关测试。高风险、跨模块、用户要求审查时才派 code-reviewer；否则主 Agent 自检即可。
    Strict Release：使用下面完整流程；下面的 per-Task review→fix 只适用于 Strict Release，不适用于 Fast Patch / Normal Feature。
